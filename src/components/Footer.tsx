...
export default function Footer() {
    ...
    return (
        <footer className="bg-slate-950/95 border-t border-slate-800/50 backdrop-blur-sm">
                ...
                <div className="max-w-7xl mx-auto">
                            ...
                            <div className="flex space-x-4">
                                {socialLinks.map((social, index) => (
                                    <a
                                        ...
                                        className={`w-10 h-10 bg-slate-800/50 border border-slate-700/50 rounded-lg flex items-center justify-center text-slate-400 ${social.color} transition-all duration-300 hover:scale-110 hover:border-slate-600/50 hover:bg-slate-700/50`}
                                        title={social.name}
                                    >
                            ...
                            <nav className="space-y-3">
                                {quickLinks.map((link, index) => (
                                    <a
                                        key={index}
                                        href={link.href}
                                        className="block text-slate-400 hover:text-white transition-colors duration-300 text-sm"
                                    >
                                        {link.name}
                                    </a>
                                ))}
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <span className="text-slate-400 text-sm">justine.liwanag@gmail.com</span>
                                </div>
                                <div className="flex items-center space-x-3">
                                    <span className="text-green-400">📱</span>
                                    <span className="text-slate-400 text-sm">+63 920 701 8305</span>
                                </div>
                                <div className="flex items-center space-x-3">
                                    <span className="text-blue-400">📍</span>
                                    <span className="text-slate-400 text-sm">Longos, Calumpit, Bulacan</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="border-t border-slate-800/50 pt-8">
                        <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                            ...
                            <button
                                onClick={scrollToTop}
                                className="flex items-center space-x-2 text-slate-400 hover:text-white transition-colors duration-300 text-sm group"
                            >
